...
lazy_static! {
    ...
    static ref CLIENT: RTCClient =
        RTCClient::new("127.0.0.1".to_string(), "default".to_string(), 60000);
}
...
async fn start_client() -> Result<()> {
    ...
    remote_connection.on_data_channel(Box::new(move |d:Arc<RTCDataChannel>| {
        let d_label = d.label().to_owned();
        let d_id = d.id();
        print!("New RTCDataChannel {} {}\n", d_label, d_id);
        ...
        Box::pin(async move{
            ...
            d.on_message(Box::new(move |msg: DataChannelMessage| {
               ...
               print!("Message from RTCDataChannel '{}': '{}'\n", d_label, msg_str);
               Box::pin(async{})
           })).await;
        })
    })).await;
...
impl RTCClient {
    ...
    async fn signal_candidate(&self, addr: &str, c: &RTCIceCandidate) -> Result<()> {
        println!(
